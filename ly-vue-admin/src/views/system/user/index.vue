  ...
  <div class="app-container">
    <el-row :gutter="20">
      <el-col :span="24" :xs="24">
        ...
        <el-table :data="userList" style="width: 100%" @selection-change="handleSelectionChange" v-loading="loading" :row-class-name="tableRowClassName">
          ...
          <el-table-column prop="roleGroups" label="角色" align="center">
            <template slot-scope="scope">
              <template v-for="group in scope.row.roleGroups">
                <template v-if="group==='超级管理员'">
                  <el-tag type="success">{{ group }}</el-tag>
                </template>
                <template v-else>
                  <el-tag v-if="group==='普通角色'" type="warning">{{ group }}</el-tag>
                  <el-tag v-else>{{ group }}</el-tag>
                </template>
              </template>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      if (!row.id && this.ids === []) {
        this.$modal.msgWarning('您暂未选择任何用户')
        return
      }
      const ids = row.id ? [row.id] : this.ids
      if (ids.includes('1')) {
        this.$modal.msgWarning('超级管理员不可以被删除')
        return
      }
      this.$modal
