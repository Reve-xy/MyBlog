...
<div class="tFriendsBox">
    <h1>友链申请格式（留言申请）
        <h3>网站名称: Rêve个人博客</h3>
        <h3>网站地址: http://127.0.0.1/Home </h3>
        <h3>网站描述: 这是Rêve的个人博客</h3>
        <h3>网站logo: http://127.0.0.1/static/img/favicon.ico</h3>

</h1>
    <el-row>
        <el-col :span="12" class="tf-item" v-for="(item,index) in friendslink" :key="'f'+index">
            <a :href="item.address" target="_blank">
                <img :src="item.logo?item.logo:'static/img/tou.png'"  :onerror="$store.state.errorImg">
                <h4>{{item.name}}</h4>
