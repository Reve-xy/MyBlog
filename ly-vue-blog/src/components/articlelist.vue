    ...
    <el-row class="sharelistBox">
      <el-col :span="24" class="s-item tcommonBox" v-for="(item,index) in articleList" :key="'article'+index">
        ...
        <header>
          ...
          <div class="ui label" v-show="item.categoryName">
...
export default {
  ...
  methods: { //事件处理器
    ...
    routeChange() {
      this.queryParams.categoryId = (this.$route.query.classId == undefined ? 0 : parseInt(this.$route.query.classId));//获取传参的classId
      this.queryParams.keyWords=(this.$route.query.search == undefined ? '' : this.$route.query.search)
